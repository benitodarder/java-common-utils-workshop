...
public class MultipartClientTest {
    ...
    public void setUp() throws HttpCommonException, IOException {
        ...
        multipartItem.setContentDisposition("form-data; name=\"file\"; filename=\"error.log\"");
        multipartItem.setContentTransferEncoding("binary");
        multipartItem.setContentType("application/octet-stream; name=error.log");
        multipartItem.setInputStream(new ByteArrayInputStream(SAMPLE_URL.getBytes()));
        request.getMultipartItems().add(multipartItem);
        headers = new HashMap<>();
    ...
    public void makeRequest_posts_body() throws HttpCommonException, IOException {
        ...
        verify(mockedOutputStream, atLeastOnce()).write((byte[]) any());
        verify(mockedOutputStream, atLeastOnce()).flush();
    }
}
